...
const sourceRoot =
  "c:/Users/idkth/Documents/scion/assets/OneDrive_2026-03-19 (2)/2. Shared with agency folder/Brands/CL";
...
products.forEach((product) => {
  ...
  if (fs.existsSync(fullSourcePath)) {
    ...
    files.forEach((file, index) => {
      if (index < 7 && (file.endsWith(".jpg") || file.endsWith(".png"))) {
        ...
        fs.copyFileSync(
          path.join(fullSourcePath, file),
          path.join(destRoot, `prod-cl-${slug}-${index + 1}${ext}`),
        );
      }
    });
  } else {
  ...
  // Update breadcrumb
  content = content.replace(
    /<a href="products-luxury.html">Maison de l'Avenir<\/a>/g,
    `<a href="products-luxury.html">Creation Lamis</a>`,
  );
