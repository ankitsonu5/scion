...
const rootCandidates = [
  ...
  path.join(
    root,
    "assets",
    "OneDrive_2026-03-19 (2)",
    "2. Shared with agency folder",
    "Brands",
  ),
];
const oneDriveRoot = rootCandidates.find((candidate) =>
...
function discoverProducts(config) {
  ...
  for (const [collectionDir, collectionMeta] of Object.entries(
...
function renderDetailPage(product, config) {
  const imagePaths = product.images.map((name) =>
    encodeRelative(`${product.imageRoot}/${name}`),
  );
  ...
  const thumbs = imagePaths
    .map(
      (
        src,
        index,
      ) => `                            <div class="pdp-thumb-v${index === 0 ? " active" : ""}" onclick="changeImage(this)">
                                <img src="${src}" alt="${escapeHtml(product.title)} view ${index + 1}">
                            </div>`,
    )
    .join("\n");
  const bullets = aboutBullets(product)
    .map(
      (bullet) => `                            <li>${escapeHtml(bullet)}</li>`,
    )
    .join("\n");
  const cards = infoCards(product)
    .map(
    ...
    .join("\n");
  const visualBullets = product.images
    .map(
      (image) =>
        `                            <li>${escapeHtml(path.basename(image, path.extname(image)))}</li>`,
    )
    .join("\n");
...
function renderCards(products) {
  return products
    .map((product) => {
      const cardImage = encodeRelative(
        `${product.imageRoot}/${product.images[0]}`,
      );
...
if (!activeBrandConfigs.length) {
  throw new Error(
    `No matching brand config for arguments: ${process.argv.slice(2).join(", ")}`,
  );
}
...
for (const config of activeBrandConfigs) {
  ...
  for (const product of products) {
    ...
    fs.writeFileSync(
      ...
      renderDetailPage(product, config),
      "utf8",
    );
  ...
  const updatedListing = replaceContainerInner(
    listingHtml,
    config.listingId,
...
fs.writeFileSync(
  path.join(root, "product_pages_list.txt"),
  `${productPages.join("\n")}\n`,
  "utf8",
);

console.log(
  `Generated or updated ${generatedPages.length} OneDrive-backed product detail pages.`,
);
