...
function extractMetaRows(html) {
    const match = html.match(/<div class="pdp-meta-amazon">[\s\S]*?<table>([\s\S]*?)<\/table>[\s\S]*?<\/div>/i);
    if (!match) return [];
    const rows = [];
    const rowRegex = /<tr>\s*<td class="label">([\s\S]*?)<\/td>\s*<td>([\s\S]*?)<\/td>\s*<\/tr>/gi;
    let rowMatch;
...
function extractAboutLines(html) {
    const aboutMatch = html.match(/<div class="about-content-text">([\s\S]*?)<\/div>/i);
...
function buildCards(rows) {
    return rows.map(({ label, value }) => `                    <div class="pdp-info-card">
...
function normalizeBuyBox(html, isCl) {
    if (isCl) {
        const amazonHrefMatch = html.match(/<a href="([^"]+)" target="_blank" class="pdp-amazon-btn">/i);
        ...
        return html.replace(
            ...
            `<div class="hybrid-buy-box">
                        ...
                        <div class="buy-box-actions">
                            ${amazonHref ? `<a href="${amazonHref}" target="_blank" class="pdp-amazon-btn">
...
function normalizeTabs(html, rows) {
    html = html.replace(/<button class="tab-btn active" onclick="openTab\(event, 'ingredients'\)">Information<\/button>/i, `<button class="tab-btn active" onclick="openTab(event, 'info')">Information</button>`);
    html = html.replace(/\s*<button class="tab-btn" onclick="openTab\(event, 'shipping'\)">Shipping & Returns<\/button>/gi, '');
    html = html.replace(/\s*<div class="tab-content" id="shipping"[\s\S]*?<\/div>\s*(?=<\/div>\s*<\/section>)/i, '\n');

    const infoMatch = html.match(/<div class="tab-content" id="(?:info|ingredients)" style="display: block;">([\s\S]*?)<\/div>\s*(?=<div class="tab-content"|<\/div>\s*<\/section>)/i);
    const description = infoMatch ? stripTags(infoMatch[1]) : '';
    const replacement = buildInfoTab(description, rows);
    html = html.replace(/<div class="tab-content" id="(?:info|ingredients)" style="display: block;">[\s\S]*?<\/div>\s*(?=<div class="tab-content"|<\/div>\s*<\/section>)/i, `${replacement}\n\n            `);
    return html;
}
...
function processFile(filePath) {
    ...
    const aboutBlock = buildAbout(aboutLines.length ? aboutLines : ['Product information available on request.']);
...
const files = fs.readdirSync(root)
    .filter((name) => name.endsWith('.html') && patterns.some((pattern) => name.startsWith(pattern)));
