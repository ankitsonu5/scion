...
const explicitSearchTerms = {
  ...
  "product-cpt-cp-np-eye-catching-brights-box-9.html":
    "eye catching nail polish pack 9",
  "product-cpt-cp-np-glitter-explosion-box-11.html": null,
  "product-cpt-cp-np-ornamental-collection-box-4.html":
    "ornamental collection nail polish pack 4",
  "product-cpt-cp-np-pastel-passion-box-8.html":
    "pastel passion nail polish pack 8",
  "product-cpt-cp-np-vibrant-tips-box-2.html":
    "vibrant tips nail polish pack 2",
  ...
  "product-pc-morocan-argan-oil-hair-cream-475ml.html":
    "moroccan argan oil hair cream 475ml",
...
const explicitItemIncludes = {
  ...
  "product-jpd-scripture-goldust-m.html":
    "scripture goldust - long lasting perfume",
  ...
  "product-jpd-scripture-weirwood-m.html":
    "scripture weirwood - long lasting perfume",
  ...
  "product-pc-combo-image.html": "3pcs regime kit",
  "product-pc-keratin-smooth-hair-cream-475ml.html":
    "keratin smooth hair cream",
  "product-pc-lanolin-hair-cream-475ml.html": "lanolin hair cream",
  "product-pc-morocan-argan-oil-hair-cream-475ml.html":
    "moroccan argan oil hair cream",
  "product-pc-volumising-treatment-hair-cream-475ml.html":
    "volumising treatment hair cream",
};
...
function normalize(value) {
  return String(value || "")
    .toLowerCase()
    .replace(
      /maison de l'avenir|creation lamis|jean paul dupont|\(jpd\)|cp trendies|paris collection|prive|privé/g,
      " ",
    )
    .replace(
      /eau de parfum|eau de toilette|gift set|all-in-one|ultimate color|for women|for men|unisex|body lotion|makeup kit|make up kit/g,
      " ",
    )
    .replace(/[^a-z0-9]+/g, " ")
    .replace(/\s+/g, " ")
...
function inferCategoryLabel(config, fileName, title) {
  ...
  if (
    config.prefix === "product-jpd-" ||
    config.prefix === "product-cl-" ||
    config.prefix === "product-ma-"
  ) {
    return "Perfume";
  }
...
function scoreRow(row, searchKey, fileName) {
  ...
  for (const candidate of candidates) {
    ...
    if (
      (fileName.startsWith("product-cpt-") ||
        fileName.startsWith("product-jpd-") ||
        fileName.startsWith("product-pc-")) &&
      coverage < 0.75
    ) {
      score = 0;
    }
...
function categoryAllowed(config, fileName, row) {
  const fileKey = fileTitleKey(fileName, config.prefix);
  const categoryText =
    `${get(row, ["Category", "Collection/Category", "Product Type"])} ${get(row, ["Amazon Item-name"])}`.toLowerCase();
  ...
  if (config.prefix === "product-pc-") {
    ...
    if (fileKey.includes("hair-food"))
      return categoryText.includes("hair food");
    if (fileKey.includes("hair-cream"))
      return categoryText.includes("hair cream");
    if (fileKey.includes("hair-oil")) return categoryText.includes("hair oil");
    if (fileKey.includes("body-lotion"))
      return categoryText.includes("body lotion");
    if (fileKey.includes("body-oil"))
      return categoryText.includes("body oil") || categoryText.includes("baby");
    if (fileKey.includes("aloe-vera"))
      return categoryText.includes("aloe vera");
    if (fileKey.includes("3pcs-regime-kit"))
      return categoryText.includes("regime kit");
  }
...
function buildAbout(row) {
  ...
  const items = (
    bullets.length ? bullets : ["Product information available on request."]
  )
...
function buildInfoCards(row, brandLabel) {
  const entries = [
    ...
    [
      "Product Type",
      get(row, ["Product Type", "Product Category", "Category"]),
    ],
    ["Target Group", get(row, ["Product Gender", "Targeted Group"])],
    ["Fragrance Family", get(row, ["Fragrance Family"])],
  ].filter(([, value]) => value);
...
function replacePageBlocks(html, metaTable, aboutBlock, infoTab, actions) {
  html = html.replace(
    /<div class="pdp-meta-amazon">[\s\S]*?<\/div>\s*<div class="pdp-divider"><\/div>/,
    `${metaTable}\n\n                    <div class="pdp-divider"></div>`,
  );
  html = html.replace(
    /<div class="pdp-about-bullets">[\s\S]*?<\/div>\s*(?=<div class="pdp-divider"><\/div>|<\/div>\s*<\/div>\s*<\/div>\s*<\/section>)/,
    `${aboutBlock}\n`,
  );
  html = html.replace(
    /<div class="tab-content" id="info" style="display: block;">[\s\S]*?<\/div>\s*(?=<div class="tab-content"|<\/div>\s*<\/section>)/,
    `${infoTab}\n\n            `,
  );

  if (actions && !html.includes("pdp-country-links")) {
    html = html.replace(
      /<div class="buy-box-actions">[\s\S]*?<\/div>\s*<\/div>\s*\n\n\s*<div class="pdp-about-bullets">/,
...
function applyGenericFallback(filePath, config, fileName) {
  ...
  const title =
    readPageTitle(html) ||
    fileTitleKey(fileName, config.prefix).replace(/-/g, " ");
  html = replacePageBlocks(
    html,
...
for (const fileName of fs
  ...
  const explicitSearch = Object.prototype.hasOwnProperty.call(
    explicitSearchTerms,
    fileName,
  )
    ...
    : undefined;
  const searchKey =
    explicitSearch === undefined ? pageTitle || fileKey : explicitSearch;
  const candidateRows = rows.filter((row) =>
    config.brands.includes(get(row, ["Brand", "Brand "])),
  );
  let bestRow = null;
  let bestScore = 0;
  ...
  if (bestRow && bestScore >= 120) {
    ...
    updated += 1;
    matched.push(
      `${fileName} <= ${get(bestRow, ["Amazon Item-name", "Amazon Product Title "])}`,
    );
  } else if (
    explicitSearch === null ||
    config.prefix === "product-cpt-" ||
    config.prefix === "product-jpd-" ||
    config.prefix === "product-pc-"
  ) {
