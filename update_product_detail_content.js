  ...
  for (const row of rows.slice(1)) {
    ...
    map.set(normalizeSlug(perfumeName), {
      description: cleanRichText(
        clean(record["Updated Description"]) || clean(record["Description"]),
      ),
      ...
      features:
        clean(record["Updated Special Feature"]) ||
        clean(record["special Feature"]),
...
for (const file of files) {
  ...
  html = html.replace(
    /\s*<div class="buy-box-stock">In Stock<\/div>\r?\n/g,
    "\r\n",
  );
